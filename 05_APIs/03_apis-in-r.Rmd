...
impeachment<-nytapi(search.terms = "impeachment", begin.date = 20190601, end.date = 20190930, key=nyt_key_1)
hits<-impeachment$response$meta$hits
print(hits)
round(hits/10)
```

After making a quick call to the API, it appears that we have 723 hits. Since the API only allows us to download 10 results at a time, we need to make 73 calls! 
```{r}
# Get all articles   
impeachment.articles<-sapply(X = 0:73, FUN = function(page){
  #cat(page, "")
  response<-tryCatch(expr = {
    r<-nytapi(search.terms = "impeachment", begin.date = 20190601, end.date = 20190930, page = page, key = nyt_key_1)
    r$response$docs
