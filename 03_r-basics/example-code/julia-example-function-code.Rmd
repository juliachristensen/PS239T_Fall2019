...
dat %<>% mutate(D = NA) %>%
  mutate(D = ifelse(match_fema_sub==1, nyt_hit_flag_st_b4decl_sub, D)) %>%
  mutate(D = ifelse(match_fema_sub==0, nyt_hit_flag_lead20_st, D))
dat %<>% mutate(Z = NA) %>%
  mutate(Z = ifelse(match_fema_sub==1, np_event_sum_b4decl_sub, Z)) %>%
  mutate(Z = ifelse(match_fema_sub==0, np_event_lead20, Z))

dat %<>%
  # Create var with days before declaration 
    mutate(days_b4_decl = as.numeric(date_declaration_sub - new_date_beg)) %>%
  # Remove rows with declaration dates before the start of the disaster
    filter((match_fema_sub==1 & days_b4_decl >= 0) | (match_fema_sub==0))
...
# p %>% gen_general_YonD_table()
# p %>% gen_general_table()

p %>% gen_general_YonD_table(output_type="latex")
p %>% gen_general_table(output_type="latex")

# p %>% gen_general_stage1_table(output_type="text", z_labels=NULL)
# p %>% gen_general_YonZ_table(output_type="text", z_labels=NULL)
...
# 
# dat %<>% mutate_at(vars(contains("_lead10")), funs(./10)) #change in diff lead
# dat %<>% mutate_at(vars(contains("_lead20")), funs(./20))
# dat %<>% mutate_at(vars(contains("_lead40")), funs(./40))
# dat %<>% mutate_at(vars(contains("_lead60")), funs(./60))
# dat %<>% mutate_at(vars(contains("b4decl_sub")), funs(avg=./(days_b4_decl+1)))

dat %<>% mutate(D = NA) %>%
  mutate(D = ifelse(match_fema_sub==1, nyt_hits_total_st_b4decl_sub, D)) %>%
  mutate(D = ifelse(match_fema_sub==0, nyt_hits_total_lead60_st, D))
dat %<>% mutate(Z = NA) %>%
  mutate(Z = ifelse(match_fema_sub==1, np_event_sum_b4decl_sub, Z)) %>%
  mutate(Z = ifelse(match_fema_sub==0, np_event_lead60, Z))
...
dat %>%
  ggplot(aes(x=np_event_lead10)) + 
    geom_histogram(bins=10, colour="white", fill="grey30") +
    scale_y_log10(labels = scales::comma) +   
    labs(title = "Distribution of News Pressure",
         subtitle = "", caption = "",  x = "", y = "") + theme_ed
plot_file_name <- paste0("../Plots/Merged/hist_disasters_np10.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")

dat %>%
  ggplot(aes(x=np_event_lead60)) + 
    geom_histogram(bins=30, colour="white", fill="grey30") +
    scale_y_log10(labels = scales::comma) +   
    labs(title = "Distribution of News Pressure",
         subtitle = "", caption = "",  x = "", y = "") + theme_ed
plot_file_name <- paste0("../Plots/Merged/hist_disasters_np60.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")

dat %>%
  ggplot(aes(x=nyt_hits_total_lead10_st)) + 
    geom_histogram(bins=10, colour="white", fill="grey30") +
    scale_y_log10(labels = scales::comma) +   
    labs(title = "Distribution of News Coverage",
         subtitle = "", caption = "",  x = "", y = "") + theme_ed
plot_file_name <- paste0("../Plots/Merged/hist_disasters_nyt10.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")

dat %>%
  ggplot(aes(x=nyt_hits_total_lead60_st)) + 
    geom_histogram(bins=30, colour="white", fill="grey30") +
    scale_y_log10(labels = scales::comma) + 
    labs(title = "Distribution of News Coverage",
         subtitle = "", caption = "",  x = "", y = "") + theme_ed 
plot_file_name <- paste0("../Plots/Merged/hist_disasters_nyt60.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")

```

### 2.3.2. X distributions
```{r}
...
dat %>%
  ggplot(aes(x=days_of_disaster)) + 
    geom_histogram(bins=30, colour="white", fill="grey30") +
    scale_y_log10(labels = scales::comma) +   
    # labs(title = "Distribution of Disaster Length in Days",
    #      subtitle = "", caption = "",  x = "", y = "") + 
    theme_ed
plot_file_name <- paste0("../Plots/Merged/hist_disasters_length.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")

dat %>%
  ggplot(aes(x=deaths_total)) + 
    geom_histogram(bins=30, colour="white", fill="grey30") +
    scale_y_log10(labels = scales::comma) +   
    # labs(title = "Distribution of Total Deaths",
    #      subtitle = "", caption = "",  x = "", y = "") + 
    theme_ed
plot_file_name <- paste0("../Plots/Merged/hist_disasters_deaths.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")
...
dat %>%
  ggplot(aes(x=injuries_total)) + 
    ...
    #      subtitle = "", caption = "",  x = "", y = "") + 
    theme_ed
plot_file_name <- paste0("../Plots/Merged/hist_disasters_injuries.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")


```

### 2.3.3. FEMA Subset and Buffers
```{r}
agg_lvl = "cz" 
dat <- load_dat_for_analysis(agg_lvl=agg_lvl)
...
dat %>% 
  mutate_at(vars(contains("match_fema_b")), funs(ifelse(is.na(.),0,.))) %>%
  group_by(year) %>%
  summarise(All = mean(match_fema_b0),
            Flood = mean(match_fema_d_b0) ) %>%
  ungroup() %>%
  mutate_at(vars(-year), ~ifelse(.==0,NA,.)) %>%
  filter_at(vars(-year), any_vars(!is.na(.))) %>%
  gather(key="Subset",value="pct",-year) %>%
  ggplot(aes(x=year, y=pct, color=Subset, group=Subset)) + 
    geom_point(aes(shape=Subset), size=2, alpha=0.75) + 
    geom_line(size=1, alpha=0.75) + 
    theme_ed +
    scale_x_discrete(breaks = levels(dat$year)[c(T, rep(F, 4))]) +
    scale_y_continuous(labels = scales::percent) +
    scale_color_viridis(option = "B", end = .8, alpha=.4, discrete = TRUE) +
    scale_shape_manual(values=c(0, 1, 2, 4)) +
    labs(title = "Avg Disasters Recieving FEMA Declaration", x = "", y = "") 
  
plot_file_name <- paste0("../Plots/Merged/line_disasters_decl.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")

### Compare buffer options
vlist <- paste0("match_fema_",c("d_b0","d_b3","d_b7","d_b14"))
dat %>% 
    group_by(year) %>%
    mutate_at(vars(match_fema_b0, match_fema_b3, match_fema_b7, match_fema_b14),
              funs(ifelse(is.na(.),0,.))) %>%
    summarise_("b0" = paste0("mean(",vlist[1],")"),
               "b3"= paste0("mean(",vlist[2],")"),
               "b7" = paste0("mean(",vlist[3],")"),
               "b14" = paste0("mean(",vlist[4],")") ) %>%
    ungroup() %>%
    mutate_at(vars(b0,b3,b7,b14), ~ifelse(.==0,NA,.)) %>%
    filter(!is.na(b0) | !is.na(b3) | !is.na(b7) | !is.na(b14)) %>%
    gather(key="Buffer",value="pct",-year) %>%
    mutate(Buffer = factor(Buffer, levels = c("b0","b3","b7","b14"),
                           labels = c("none","3 days","1 week","2 weeks"))) %>% 
    ggplot(aes(x=year, y=pct, color=Buffer, group=Buffer)) + 
      geom_point(aes(shape=Buffer), size=2, alpha=0.75) + 
      geom_line(size=1, alpha=0.75) + 
      theme_ed +
      scale_x_discrete(breaks = levels(dat$year)[c(T, rep(F, 4))]) +
      scale_y_continuous(labels = scales::percent) +
...
plot_file_name <- paste0("../Plots/Merged/line_disasters_decl_sub_buffers.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")
```

### 2.3.3. Time series: flood events by year
```{r}

agg_lvl = "cz" 
dat <- load_dat_for_analysis(agg_lvl=agg_lvl)

### Scatter Plot of Disasters by Year and incident type 
dat %>%
    mutate(year = as.integer(year(new_date_beg))) %>% 
  # Remove duplicates for each disaster
    distinct(stormid, year, FIPS) %>%
  # Group by date and summarize the number of disasters 
    group_by(year) %>%
    summarize(count = n()) %>%
    ungroup() %>%
  # Plot 
    ggplot(aes(x=year, y=count)) + 
      ...
      geom_point(na.rm = TRUE, colour = "dark blue", size=2) + 
      theme_ed +
      ggtitle("Storm Data Flood Disasters by Year") + 
      # scale_x_continuous(breaks = c(1990:2018)[seq(1, 2018-1950+1, 2)]) +
      xlab("Year") + ylab(NULL) #+ scale_y_continuous(trans=log10_trans()) #Log transformation

ggsave("../Plots/Storm/scatter_count_by_year.pdf", 
       plot = last_plot(), width=8, height=6, units="in") 

```

### 2.3.4. Days between Start of Disaster and FEMA Declaration
```{r}
agg_lvl = "cz" 
fema_sub <- TRUE
dat <- load_dat_for_analysis_1to1(agg_lvl=agg_lvl, fema_sub=fema_sub)
...
dat %>% filter(days_b4_decl < -100) %>%
  dplyr::select(new_date_beg,
         new_date_end,
         disaster_num_sub,
         date_declaration_sub,
         match_fema_sub, 
         np_event_lead60,
         nyt_hits_total_lead60_st,         
         np_event_sum_b4decl_sub,
         nyt_hits_total_st_b4decl_sub,
         days_b4_decl, days_of_disaster) %>% 
  print.data.frame()

dat %>%
  filter(disaster_num_sub != 829) %>% 
  ggplot(aes(x=days_b4_decl)) + 
    geom_histogram(binwidth=5, colour="white", fill="grey30") +
    labs(title = "Days between Start of Disaster and FEMA Declaration",
         subtitle = "", caption = "",  x = "", y = "") + theme_ed +
    geom_vline(aes(xintercept=0), color="red", size=1)
plot_file_name <- paste0("../Plots/Merged/hist_disaster_decl_delay_sub.pdf")
ggsave(plot_file_name, plot = last_plot(), width=8, height=6, units="in")

```

### 2.3.4. 1to1 FEMA Declaration
```{r}
agg_lvl = "cz" 
fema_sub <- TRUE
dat <- load_dat_for_analysis_1to1(agg_lvl=agg_lvl, fema_sub=fema_sub)

sum(dat$match_fema_sub) / nrow(dat)
